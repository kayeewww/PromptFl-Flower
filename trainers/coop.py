#!/usr/bin/env python3
"""
CoOp Trainer for PromptFL with Flower - Context Optimization
"""
...
class CoOpFlower:
    """CoOp trainer - Context Optimization"""
    ...
    def build_model(self):
        ...
        print(f"Loading CLIP for CoOp (backbone: {cfg.MODEL.BACKBONE.NAME})")
        clip_model, _ = clip.load(cfg.MODEL.BACKBONE.NAME, device="cpu")
        
        if cfg.TRAINER.COOP.PREC == "fp32":
            clip_model.float()

        print("Building CoOp CLIP")
        self.model = CoOpCLIP(cfg, classnames, clip_model)
    ...
    def train(self, idx=None, global_epoch=None, is_fed=False):
        ...
        print(f"CoOp Client {idx} - Epoch {global_epoch}: Loss={losses.avg:.4f}, Acc={accuracies.avg:.4f}")
    ...
    def test(self, is_global=False, current_epoch=None):
        ...
        print(f"CoOp Test - Epoch {current_epoch}: Acc={accuracy:.4f}, Loss={avg_loss:.4f}")
