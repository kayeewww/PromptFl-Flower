#!/usr/bin/env python3
"""
PromptFL with Flower - Configuration
基于Dassl配置系统的Flower版本配置
"""
...
# PromptFL specific dataset configs
_C.DATASET.SUBSAMPLE_CLASSES = "all"  # all, base or new
_C.DATASET.USERS = 2  # number of clients
_C.DATASET.IID = False  # is iid
_C.DATASET.USEALL = False # use all data for training instead of few shot
_C.DATASET.REPEATRATE = 0.0 # repeat rate on each client
...
# PromptFL specific optimization configs
_C.OPTIM.ROUND = 10 # global round
...
# PromptFL trainer
_C.TRAINER.PROMPTFL = CN()
_C.TRAINER.PROMPTFL.N_CTX = 16  # number of context vectors
_C.TRAINER.PROMPTFL.CSC = False  # class-specific context
_C.TRAINER.PROMPTFL.CTX_INIT = ""  # initialization words
_C.TRAINER.PROMPTFL.PREC = "fp16"  # fp16, fp32, amp
_C.TRAINER.PROMPTFL.CLASS_TOKEN_POSITION = "end"  # 'middle' or 'end' or 'front'

# CoOp trainer
_C.TRAINER.COOP = CN()
_C.TRAINER.COOP.N_CTX = 16  # number of context vectors
_C.TRAINER.COOP.CSC = False  # class-specific context
_C.TRAINER.COOP.CTX_INIT = ""  # initialization words
_C.TRAINER.COOP.PREC = "fp16"  # fp16, fp32, amp
_C.TRAINER.COOP.CLASS_TOKEN_POSITION = "end"  # 'middle' or 'end' or 'front'

# Baseline trainer
_C.TRAINER.BASELINE = CN()
_C.TRAINER.BASELINE.PREC = "fp16"  # fp16, fp32, amp
...
def extend_cfg(cfg):
    """
    Add new config variables for PromptFL.
    
    This function extends the base configuration with PromptFL-specific
    settings that are not in the original Dassl configuration.
    """
    # All PromptFL-specific configs are already included in the default config above
    pass
